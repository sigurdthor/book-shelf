...
object ErrorHandler {
  ...
  def withErrorCodeExtensions[R](
      interpreter: GraphQLInterpreter[R, Throwable]): GraphQLInterpreter[R, CalibanError] =
    interpreter.mapError {
      case err @ ExecutionError(_, _, _, Some(error: StatusException), _) =>
        err.copy(
          extensions = Some(ObjectValue(
            List(("errorCode" -> StringValue(String.valueOf(error.getStatus.getCode.value())))))))
      case err: ExecutionError =>
        err.copy(
          extensions = Some(ObjectValue(List(("errorCode", StringValue("EXECUTION_ERROR"))))))
      case err: ValidationError =>
        err.copy(
          extensions = Some(ObjectValue(List(("errorCode", StringValue("VALIDATION_ERROR"))))))
